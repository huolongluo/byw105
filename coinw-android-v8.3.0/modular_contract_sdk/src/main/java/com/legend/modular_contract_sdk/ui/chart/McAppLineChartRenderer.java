 ...
 */
public class McAppLineChartRenderer extends LineChartRenderer {
    private float[] mCirclesBuffer = new float[2];

    public McAppLineChartRenderer(LineDataProvider chart, ChartAnimator animator, ViewPortHandler viewPortHandler) {
...
//            mHighlightLinePath.moveTo(mViewPortHandler.contentLeft(), y);
    ...
    public void drawLastPointCircle(Canvas c) {
        McOnLastPointChangeListener listener = null;
        if (mChart instanceof McAppCombinedChart) {
            listener = ((McAppCombinedChart) mChart).getOnLastPointChangeListener();
        }
    ...
    private void drawLastValue(Canvas c, int digit, ILineScatterCandleRadarDataSet dataSet, Transformer trans, float lastValue, int lastIndex) {
        ...
        if (lastValue != 0) {
            ...
            String lastString = NumberStringUtil.formatAmount(lastValueD,digit);
            ...
            if (mViewPortHandler.isInBoundsRight(lastX + lastStringWidth)) { // 最后一个点是否在画布范围内
                ...
                // 在范围内
                mLastPriceLinePaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_last_value_color));
                c.drawLine(lastX, lineY, c.getWidth() - lastStringWidth - padding, lineY, mLastPriceLinePaint);
                mScrollToRightRect.set(0, 0, 0, 0);
                // 绘制值
                mLastPriceTextPaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_chart_background));
                c.drawRect(
                        ...
                        lastY + (lastStringHeight + padding) / 2F,
                        mLastPriceTextPaint);
                mLastPriceTextPaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_last_value_color));
                ...
                // 绘制线
                float lineY = Math.max(lastY, (lastStringHeight + padding) / 2);
                mLastPriceLinePaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_axis_color));
                c.drawLine(0, lineY, c.getWidth(), lineY, mLastPriceLinePaint);
                // 绘制背景
                mLastPriceTextPaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_marker_color2));
                ...
                mLastPriceBorderPaint.setColor(ContextCompat.getColor(context, R.color.mc_sdk_axis_color));
