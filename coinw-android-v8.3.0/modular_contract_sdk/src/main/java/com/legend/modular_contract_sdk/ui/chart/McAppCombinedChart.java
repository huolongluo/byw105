...
public class McAppCombinedChart extends CombinedChart {
    ...
    private McOnLastPointChangeListener mOnLastPointChangeListener;

    public McAppCombinedChart(Context context) {
        this(context, null);
    }

    public McAppCombinedChart(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public McAppCombinedChart(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void init() {
        super.init();
        mRenderer = new McAppCombinedChartRenderer(this, mAnimator, mViewPortHandler);
    }
    ...
    public void setOnLastPointChangeListener(McOnLastPointChangeListener listener) {
        mOnLastPointChangeListener = listener;
    }

    public McOnLastPointChangeListener getOnLastPointChangeListener() {
    ...
    public void setData(CombinedData data) {
        ...
        ((McAppCombinedChartRenderer) mRenderer).createRenderers();
...
//            if (mMarker instanceof LineChartYMarkerView) {
            McLineChartYMarkerView yMarker = (McLineChartYMarkerView) mMarker;
            McLineChartXMarkerView xMarker = (McLineChartXMarkerView) mXMarker;
            ...
            if (mIsFullScreen) {
                ...
                if (mIsShowSubIndex) {
                    if (mIsShowBar) {
                        chartHeight = 2 * getResources().getDimensionPixelSize(R.dimen.mc_sdk_bottom_chart_height);
                    } else {
                        chartHeight = getResources().getDimensionPixelSize(R.dimen.mc_sdk_bottom_chart_height);
                    }
                } else {
                    if (mIsShowBar) {
                        chartHeight = getResources().getDimensionPixelSize(R.dimen.mc_sdk_bottom_chart_height);
