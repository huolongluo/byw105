...
public class RxWebSocketUtil {
    private static RxWebSocketUtil instance = new RxWebSocketUtil();
    ...
    public Observable<WebSocketInfo> getWebSocketInfo(final String url, final long timeout, final TimeUnit timeUnit, int count) {
        Observable<WebSocketInfo> observable = observableMap.get(url);
        if (observable == null) {
            observable = Observable.create(new WebSocketOnSubscribe(url))
                    //自动重连
                    .timeout(timeout, timeUnit).retry(count)
                    //共享
                    .doOnUnsubscribe(new Action0() {
                        @Override
                        public void call() {
    ...
    public Observable<WebSocketInfo> getWebSocketInfo(String url) {
        Logger.getInstance().debug("RxWebSocketUtil", "getWebSocketInfo1", new Exception());
        return getWebSocketInfo(url, 30, TimeUnit.DAYS);
    }

    public Observable<String> getWebSocketString(String url) {
        Logger.getInstance().debug("RxWebSocketUtil", "getWebSocketInfo2", new Exception());
        return getWebSocketInfo(url).map(new Func1<WebSocketInfo, String>() {
            @Override
    ...
    public Observable<ByteString> getWebSocketByteString(String url) {
        return getWebSocketInfo(url).map(new Func1<WebSocketInfo, ByteString>() {
            @Override
    ...
    public Observable<WebSocket> getWebSocket(String url) {
        return getWebSocketInfo(url).map(new Func1<WebSocketInfo, WebSocket>() {
            @Override
    ...
    public void asyncSend(String url, final String msg) {
        getWebSocket(url).first().subscribe(new Action1<WebSocket>() {
            @Override
    ...
    private Request getRequest(String url) {
        ...
        return new Request.Builder().get().url(url)//
                // .addHeader("language", language)//语言
                .addHeader("selectType", PricingMethodUtil.getPricingSelectType())//计价方式
                .addHeader("clientTag", "android")//终端
                .addHeader("appVersion", BuildConfig.VERSION_CODE + "")//当前版本号
                .addHeader("deviceId", deviceId)//设备指纹唯一标识符
                .addHeader(SSLHelper.getHeadKey(), SSLHelper.getHeadValue(11, url.replace("wss:", "https:").replace("ws:", "http:")))//
                .build();
    }
