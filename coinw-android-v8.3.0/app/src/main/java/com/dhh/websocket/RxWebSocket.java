...
public final class RxWebSocket {
    ...
    public static Observable<WebSocketInfo> get(String url) {
        Logger.getInstance().debug("RxWebSocket", "Observable-url: " + url, new Exception());
        return RxWebSocketUtil.getInstance().getWebSocketInfo(url);
    }

    public static Observable<WebSocketInfo> get(String url, int count) {
        return RxWebSocketUtil.getInstance().getWebSocketInfo(url, 30, TimeUnit.DAYS, count);
    }
