package com.blankj.utilcodes.utils;
...
public class ToastUtils {
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(BaseApp.getSelf().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(BaseApp.getSelf().getResources().getString(resId), args), duration);
    }
    ...
    private static void showToast(CharSequence text, int duration) {
        ...
        if(!Util.isShowErrorMsg(text.toString())){//停机维护屏蔽某些文案
            return;
        }
        Logger.getInstance().debug(TAG,"show message:"+text);
        if (isJumpWhenMore) cancelToast();
        if (sToast == null) {
            sToast = Toast.makeText(BaseApp.getSelf(), text, duration);
        } else {
            sToast.setText(text);
