...
public class VerificationCodeView extends RelativeLayout {
    ...
    private void initTextViews(Context context, int etNumber, int etWidth, Drawable etDividerDrawable, float etTextSize, int etTextColor) {
        ...
        int margin= (int) ((screenWidth-2*dp2px(16,context)-6*dp2px(40,context))/5);//计算间隙的宽度
        for (int i = 0; i < mPwdTextViews.length; i++) {
            ...
            LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(Util.dp2px(context,40),
                    LayoutParams.MATCH_PARENT);
            if(i==mPwdTextViews.length-1){
