...
//        computeOBV(list)
     ...
     */
    private fun calculateMA(dayCount: Int, data: List<HisData>): List<Double> {
    ...
    private fun calculateVolMA(dayCount: Int, data: List<HisData>): List<Double> {
        val result = ArrayList<Double>(data.size)
        var i = 0
        val len = data.size
        while (i < len) {
     ...
     * 计算 MACD
     */
    private fun computeMACD(entries: List<HisData>) {
        var ema12 = 0.0
        var ema26 = 0.0
        var diff = 0.0
        var dea = 0.0
    ...
    private fun computeBOLL(data: List<HisData>) {
     ...
     * 计算 RSI
     */
    private fun computeRSI(entries: List<HisData>) {
        var rsi1 = 0.0
        var rsi2 = 0.0
        var rsi3 = 0.0
        var rsi1ABSEma = 0.0
     ...
     * 计算 KDJ
     */
    private fun computeKDJ(entries: List<HisData>) {
        var k = 0.0
        var d = 0.0
    ...
    private fun computeSAR(entries: List<HisData>) {
    ...
    private fun computeWR(entries: List<HisData>) {
    ...
    private fun computeOBV(entries: List<HisData>) {
        val dayCount = 30
        val output = DoubleArray(entries.size)
        val begin = MInteger()
        val length = MInteger()
    ...
    fun formatVolume(volume: Double): String {
        ...
        return if (volume > 1000) {
            NorUtils.NumberFormat(2).format(volume / 1000) + "K"
        } else NorUtils.NumberFormat(2).format(volume)
    }
     ...
     */
    @JvmStatic
    fun copyIndex(oldData: HisData, newData: HisData) {
