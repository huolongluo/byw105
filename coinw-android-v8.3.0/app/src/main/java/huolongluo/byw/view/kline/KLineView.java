 ...
 */
public class KLineView extends LinearLayout implements OnChartValueSelectedListener, CoupleChartGestureListener.OnAxisChangeListener {
    public static final int TYPE_K = 111;
    public static final int TYPE_LINE = 222;
    public static final Comparator<HisData> HIS_DATA_COMPARATOR = (o1, o2) -> (int) (o1.getDate() - o2.getDate());
    /**
     * 图表的类型
     */
     ...
     * 分时图小红点动画
     */
    private RippleBackground mPointView;
    ...
    public KLineView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mValueColor = ContextCompat.getColor(mContext, R.color.white);
        ...
        mBorderColor = ContextCompat.getColor(getContext(), R.color.chart_border_color);
    ...
    public void setColor(int upColor, int dropColor){
        mDecreasingColor = upColor;
        mIncreasingColor = dropColor;
        invalidate();
    }
    ...
    protected void initChartPrice() {
        ...
        LineChartXMarkerView mvx = new LineChartXMarkerView(mContext, mData, mDateFormat);
        mvx.setChartView(mChartPrice);
        mChartPrice.setXMarker(mvx);
        LineChartYMarkerView mvy = new LineChartYMarkerView(mContext, mDigits);
        ...
        axisRightPrice.setValueFormatter((value, axis) -> NorUtils.NumberFormat(mDigits).format(value));
...
//        设置标签Y渲染器
    ...
    protected void initChartVolume(AppCombinedChart chart) {
        ...
        if (isVolumeChart) {
            ...
            axisRightVolume.setValueFormatter((value, axis) -> DataUtils.formatVolume(value * rate));
        }
    }

    private void initChartListener() {
        mCoupleChartGestureListener = new CoupleChartGestureListener(this, mChartPrice, mChartVolume, mChartIndex);
        mChartPrice.setOnChartGestureListener(mCoupleChartGestureListener);
        mChartPrice.setOnChartValueSelectedListener(this);
        runMainThread(mChartPrice, () -> mChartPrice.setOnTouchListener(new ChartInfoViewHandler(mChartPrice, this::onNothingSelected, () -> {
    ...
    public void initChartKData(List<HisData> hisDatas, boolean isShowBar) {
        mIsInit = true;
        mChartType = TYPE_K;
        ...
        mMainIndex = SPUtils.getKLineMainIndex(mContext);
        mSubIndex = SPUtils.getKLineSubIndex(mContext);
        setOffset(isShowBar);
    ...
    public void initChartPriceData(List<HisData> hisDatas, boolean isShowBar) {
        mIsInit = true;
    ...
    private BarDataSet setBar(ArrayList<BarEntry> barEntries) {
        ...
        barDataSet.setHighLightColor(getResources().getColor(R.color.highlight_color_vertical));
    ...
    private LineDataSet setLine(int type, ArrayList<Entry> lineEntries) {
        ...
        if (type == NORMAL_LINE) {
            ...
            // 这里设置是否显示红点
            lineDataSetMa.setCircleColor(ContextCompat.getColor(mContext, R.color.white));
            lineDataSetMa.setHighLightColor(ContextCompat.getColor(mContext, R.color.highlight_color_vertical));
            lineDataSetMa.setHighlightLineWidth(6);
            lineDataSetMa.setHighlightHorizontalLineWidth(0.34F);
            lineDataSetMa.setHighLightHorizontalColor(ContextCompat.getColor(mContext, R.color.highlight_color_horizontal));
            lineDataSetMa.setDrawFilled(true);
            lineDataSetMa.setFillDrawable(ContextCompat.getDrawable(mContext, R.drawable.bg_time_line_chart));
            ...
            lineDataSetMa.setColor(getResources().getColor(R.color.index_color1));
            lineDataSetMa.setCircleColor(mTransparentColor);
            lineDataSetMa.setHighlightEnabled(false);
            lineDataSetMa.setVisible(false);
        } else if (type == MA10) {
            lineDataSetMa.setColor(getResources().getColor(R.color.index_color2));
            lineDataSetMa.setCircleColor(mTransparentColor);
            lineDataSetMa.setHighlightEnabled(false);
            lineDataSetMa.setVisible(false);
        } else if (type == MA30) {
            lineDataSetMa.setColor(getResources().getColor(R.color.index_color3));
            lineDataSetMa.setCircleColor(mTransparentColor);
            lineDataSetMa.setHighlightEnabled(false);
            lineDataSetMa.setVisible(false);
            ...
            lineDataSetMa.setCircleColor(getResources().getColor(R.color.index_color1));
    ...
    public CandleDataSet setKLine(int type, ArrayList<CandleEntry> lineEntries) {
        ...
        set1.setHighLightColor(ContextCompat.getColor(getContext(), R.color.highlight_color_vertical));
        set1.setHighlightHorizontalLineWidth(0.33F);
        set1.setHighLightHorizontalColor(ContextCompat.getColor(getContext(), R.color.highlight_color_horizontal));
        if (type != NORMAL_LINE) {
    ...
    public void addKDataList(List<HisData> hisDatas) {
        if (!mIsInit) {
            return;
        }
        KSetHolder KSetHolder = new KSetHolder();
        for (HisData h : hisDatas) {
            int hisIndex = mData.lastIndexOf(h);
            if (hisIndex >= 0) {
                HisData oldData = mData.get(hisIndex);
                DataUtils.copyIndex(oldData, h);
            ...
            mChartPrice.setRealCount(mData.size());
            HisData hisData = mData.get(hisIndex >= 0 ? hisIndex : mData.size() - 1);
    ...
    public void addKDataListToPre(List<HisData> hisDatas, boolean moveToLast) {
        if (!mIsInit) {
            return;
        }
    ...
    public void addLineDataListToPre(List<HisData> hisDatas, boolean moveToLast) {
        if (!mIsInit) {
            return;
        }
        ((BarLineChartTouchListener) (mChartPrice.getOnTouchListener())).stopDeceleration();
    ...
    public void addLineDataList(List<HisData> hisDatas) {
        if (!mIsInit) {
            return;
        }
    ...
    private void setOffset(boolean isShowBar) {
        ...
        int bottomOffset = getResources().getDimensionPixelSize(R.dimen.chart_xaxis_height);
        float topOffset = Utils.convertDpToPixel(20);
        ViewGroup.MarginLayoutParams waterMarkLp = (MarginLayoutParams) mWaterMarkView.getLayoutParams();
        if (SubIndex.NONE.equals(mSubIndex)) { // 不显示子指标
    ...
    public HisData getLastMinsData(long date) {
        try {
     ...
     * 显示K线副指标
     */
    public void showSubIndex(SubIndex subIndex) {
        mSubIndex = subIndex;
    ...
    private void setMainIndexByData(HisData hisData) {
        ...
        switch (mMainIndex) {
            case MA:
                CharSequence ma5 = StringUtil.getStringByColor(mContext, "MA5:" + formatIndexValue(hisData.getMa5(), mDigits) + "  ", R.color.index_color1);
                CharSequence ma10 = StringUtil.getStringByColor(mContext, "MA10:" + formatIndexValue(hisData.getMa10(), mDigits) + "  ", R.color.index_color2);
                CharSequence ma30 = StringUtil.getStringByColor(mContext, "MA30:" + formatIndexValue(hisData.getMa30(), mDigits) + "  ", R.color.index_color3);
                text = TextUtils.concat(ma5, ma10, ma30);
                break;
            case BOLL:
                CharSequence ub = StringUtil.getStringByColor(mContext, "UB:" + formatIndexValue(hisData.getUp(), mDigits) + "  ", R.color.index_color1);
                CharSequence boll = StringUtil.getStringByColor(mContext, "BOLL:" + formatIndexValue(hisData.getMb(), mDigits) + "  ", R.color.index_color2);
                CharSequence lb = StringUtil.getStringByColor(mContext, "LB:" + formatIndexValue(hisData.getDn(), mDigits) + "  ", R.color.index_color3);
                text = TextUtils.concat(boll, ub, lb);
                break;
            case EMA:
                CharSequence ema5 = StringUtil.getStringByColor(mContext, "EMA5:" + formatIndexValue(hisData.getEma5(), mDigits) + "  ", R.color.index_color1);
                CharSequence ema10 = StringUtil.getStringByColor(mContext, "EMA10:" + formatIndexValue(hisData.getEma10(), mDigits) + "  ", R.color.index_color2);
                CharSequence ema20 = StringUtil.getStringByColor(mContext, "EMA30:" + formatIndexValue(hisData.getEma30(), mDigits) + "  ", R.color.index_color3);
                text = TextUtils.concat(ema5, ema10, ema20);
                break;
            case SAR:
                CharSequence sar = StringUtil.getStringByColor(mContext, "SAR(2,20) SAR:" + formatIndexValue(hisData.getSar(), mDigits) + "  ", R.color.index_color1);
    ...
    private void setSubIndexByData(HisData hisData) {
        if (mSubIndex == null) {
            return;
        }
        CharSequence text = null;
        switch (mSubIndex) {
            case MACD:
                CharSequence macdName = StringUtil.getStringByColor(mContext, "MACD(12,26,9)  ", R.color.index_name_color);
                CharSequence macd = StringUtil.getStringByColor(mContext, "MACD:" + formatIndexValue(hisData.getMacd(), mDigits) + "  ", R.color.index_color1);
                CharSequence dif = StringUtil.getStringByColor(mContext, "DIF:" + formatIndexValue(hisData.getDiff(), mDigits) + "  ", R.color.index_color2);
                CharSequence dea = StringUtil.getStringByColor(mContext, "DEA:" + formatIndexValue(hisData.getDea(), mDigits) + "  ", R.color.index_color3);
                text = TextUtils.concat(macdName, macd, dif, dea);
                break;
            case KDJ:
                // kdj保留两位精度
                CharSequence kdjName = StringUtil.getStringByColor(mContext, "KDJ(9,3,3)  ", R.color.index_name_color);
                CharSequence k = StringUtil.getStringByColor(mContext, "K:" + formatIndexValue(hisData.getK(), 2) + "  ", R.color.index_color1);
                CharSequence d = StringUtil.getStringByColor(mContext, "D:" + formatIndexValue(hisData.getD(), 2) + "  ", R.color.index_color2);
                CharSequence j = StringUtil.getStringByColor(mContext, "J:" + formatIndexValue(hisData.getJ(), 2) + "  ", R.color.index_color3);
                text = TextUtils.concat(kdjName, k, d, j);
                break;
            case RSI:
                CharSequence rsi6 = StringUtil.getStringByColor(mContext, "RSI6:" + formatIndexValue(hisData.getRsi1(), mDigits) + "  ", R.color.index_color1);
                CharSequence rsi12 = StringUtil.getStringByColor(mContext, "RSI12:" + formatIndexValue(hisData.getRsi2(), mDigits) + "  ", R.color.index_color2);
                CharSequence rsi24 = StringUtil.getStringByColor(mContext, "RSI24:" + formatIndexValue(hisData.getRsi3(), mDigits) + "  ", R.color.index_color3);
                text = TextUtils.concat(rsi6, rsi12, rsi24);
                break;
            case OBV:
                CharSequence obv = StringUtil.getStringByColor(mContext, "OBV(30) OBV:" + formatIndexValue(hisData.getObv(), mDigits) + "  ", R.color.index_color1);
                CharSequence maobv = StringUtil.getStringByColor(mContext, "MAOBV:" + formatIndexValue(hisData.getMaobv(), mDigits) + "  ", R.color.index_color2);
                text = TextUtils.concat(obv, maobv);
                break;
            case WR:
                CharSequence wr10 = StringUtil.getStringByColor(mContext, "WR10:" + formatIndexValue(hisData.getWr10(), mDigits) + "  ", R.color.index_color1);
                CharSequence wr6 = StringUtil.getStringByColor(mContext, "WR6:" + formatIndexValue(hisData.getWr6(), mDigits) + "  ", R.color.index_color2);
    ...
    private void setVolumeByData(HisData hisData) {
        if (mTvVolumeIndex == null) {
            return;
        }
        CharSequence text = null;
        CharSequence vol = StringUtil.getStringByColor(mContext, "VOL:" + DataUtils.formatVolume(hisData.getVol()) + "  ", R.color.index_name_color);
        if (mChartType == TYPE_K) {
            CharSequence ma5 = StringUtil.getStringByColor(mContext, "MA5:" + DataUtils.formatVolume(hisData.getVolMa5()) + "  ", R.color.index_color1);
            CharSequence ma10 = StringUtil.getStringByColor(mContext, "MA10:" + DataUtils.formatVolume(hisData.getVolMa10()) + "  ", R.color.index_color2);
    ...
    private String formatIndexValue(double value, int digits) {
        ...
        return NorUtils.NumberFormat(digits).format(value);
    }
