...
public class MACDEntity
{
     ...
     */
    public MACDEntity(List<Float> closeListsFinal)
    {
        ...
        float dEA = 0.0f;
        float mACD = 0.0f;
        if (closeListsFinal != null && closeListsFinal.size() > 0)
        {
            for (int i = 0; i < closeListsFinal.size(); i++)
            {
                close = closeListsFinal.get(i);
                if (i == 0)
                {
                    eMA12 = close;
                    eMA26 = close;
