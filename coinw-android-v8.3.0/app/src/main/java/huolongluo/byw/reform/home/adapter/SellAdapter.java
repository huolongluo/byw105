 ...
 */
public class SellAdapter extends RecyclerView.Adapter<SellAdapter.BuyViewHolder> {

    public interface OnClickListener {

        void onPrice(String onePrice, String totalNumber);
    }
    ...
    public SellAdapter(List<BuyOrderBean> list, Context context) {
    ...
    public BuyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.trade_sell_item1, parent, false);
    ...
    public void onBindViewHolder(BuyViewHolder holder, int position) {
        if (isReset) {
            holder.position_tv.setText("卖" + (position + 1) + "");
...
//        Log.e("progressbar1","totalAmount= "+totalAmount+ " ;getAmount= "+getProgress(Double.parseDouble(bean.getAmount())));
        holder.progressbar.setProgress(getProgress(DoubleUtils.parseDouble(bean.getAmount())));
    ...
    public double getMaxAmountBuy() {
        double max = 0d;
        int length;
        if (list.size() < SellAdapter.length) {
            length = list.size();
        } else {
            length = SellAdapter.length;
        }
