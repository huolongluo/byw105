...
public class MessageListPanelEx {
    ...
    //过滤消息
    private List<IMMessage> filterMsgs = new ArrayList<>();
    private List<IMMessage> removeMessages;
        ...
        public void onItemChildClick(IRecyclerView adapter2, View view, int position) {
            ...
            if (LinkElement.TYPE_URL.equals(element.getType())) {
                ...
                if (message != null) {
                    ...
                    IMMessage robotMsg = MessageBuilder.createRobotMessage(message.getSessionId(), message.getSessionType(), robotAccount, robotLinkView.getShowContent(), RobotMsgType.LINK, "", element.getTarget(), element.getParams());
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private RequestCallback<List<IMMessage>> callback = new RequestCallbackWrapper<List<IMMessage>>() {
            ...
            public void onResult(int code, List<IMMessage> messages, Throwable exception) {
                //10-25 17:00:16.458 D/AndroidRuntime(11797): Shutting down VM
                //10-25 17:00:16.460 E/[ui]APP (11797): 1/************* crash *************
                //10-25 17:00:16.460 E/[ui]APP (11797): ** Thread: huolongluo.byw/main **
                //10-25 17:00:16.460 E/[ui]APP (11797): java.lang.NullPointerException: Attempt to invoke interface method 'java.util.Iterator java.util.List.iterator()' on a null object reference
                //10-25 17:00:16.460 E/[ui]APP (11797): at com.netease.nim.uikit.business.session.module.list.MessageListPanelEx$MessageLoader$1.onResult(MessageListPanelEx.java:583)
                //10-25 17:00:16.460 E/[ui]APP (11797): at com.netease.nim.uikit.business.session.module.list.MessageListPanelEx$MessageLoader$1.onResult(MessageListPanelEx.java:579)
                if (messages == null) {
                    return;
                }
                filterMsgs.clear();
                for (IMMessage message : messages) {
                    ...
                    if (null != message.getRemoteExtension() && null != message.getRemoteExtension().get("orderNo")) {
                        if (P2PMessageActivity.orderNo.replace("#", "").equals(message.getRemoteExtension().get("orderNo"))) {
        ...
        private void loadAnchorContext() {
            ...
            NIMClient.getService(MsgService.class).queryMessageListEx(anchor(), direction, loadMsgCount, true).setCallback(new RequestCallbackWrapper<List<IMMessage>>() {
                @Override
                public void onResult(int code, List<IMMessage> messages, Throwable exception) {
                    if (code != ResponseCode.RES_SUCCESS || exception != null) {
                        return;
        ...
        private void loadFromLocal(QueryDirectionEnum direction) {
            ...
            NIMClient.getService(MsgService.class).queryMessageListEx(anchor(), direction, loadMsgCount, true).setCallback(callback);
        }

        private void loadFromRemote() {
            this.direction = QueryDirectionEnum.QUERY_OLD;
            NIMClient.getService(MsgService.class).pullMessageHistory(anchor(), loadMsgCount, true).setCallback(callback);
        }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void showReDownloadConfirmDlg(final IMMessage message) {
            ...
            final EasyAlertDialog dialog = EasyAlertDialogHelper.createOkCancelDiolag(container.activity, null, container.activity.getString(R.string.repeat_download_message), true, listener);
            dialog.show();
        }
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            //10-16 09:52:48.954 E/AndroidRuntime(15319): FATAL EXCEPTION: main
            //10-16 09:52:48.954 E/AndroidRuntime(15319): Process: huolongluo.byw, PID: 15319
            //10-16 09:52:48.954 E/AndroidRuntime(15319): java.lang.NullPointerException: Attempt to invoke interface method 'boolean com.netease.nim.uikit.business.session.module.MsgForwardFilter.shouldIgnore(com.netease.nimlib.sdk.msg.model.IMMessage)' on a null object reference
            //10-16 09:52:48.954 E/AndroidRuntime(15319):     at com.netease.nim.uikit.business.session.module.list.MessageListPanelEx$MsgItemEventListener.prepareDialogItems(MessageListPanelEx.java:880)
        ...
        private boolean enableRevokeButton(IMMessage selectedItem) {
            //10-16 09:26:15.927 E/AndroidRuntime(27448): FATAL EXCEPTION: main
            //10-16 09:26:15.927 E/AndroidRuntime(27448): Process: huolongluo.byw, PID: 27448
            //10-16 09:26:15.927 E/AndroidRuntime(27448): java.lang.NullPointerException: Attempt to invoke interface method 'boolean com.netease.nim.uikit.business.session.module.MsgRevokeFilter.shouldIgnore(com.netease.nimlib.sdk.msg.model.IMMessage)' on a null object reference
            //10-16 09:26:15.927 E/AndroidRuntime(27448): at com.netease.nim.uikit.business.session.module.list.MessageListPanelEx$MsgItemEventListener.enableRevokeButton(MessageListPanelEx.java:913)
            try {
                if (selectedItem.getStatus() == MsgStatusEnum.success && NimUIKitImpl.getMsgRevokeFilter() != null && !NimUIKitImpl.getMsgRevokeFilter().shouldIgnore(selectedItem) && !recordOnly) {
        ...
        private void showResendConfirm(final IMMessage message) {
            ...
            final EasyAlertDialog dialog = EasyAlertDialogHelper.createOkCancelDiolag(container.activity, null, container.activity.getString(R.string.repeat_send_message), true, listener);
            dialog.show();
        }

        // 长按菜单项--复制
        private void longClickItemCopy(final IMMessage item, CustomAlertDialog alertDialog, MsgTypeEnum msgType) {
            if (msgType == MsgTypeEnum.text || (msgType == MsgTypeEnum.robot && item.getAttachment() != null && !((RobotAttachment) item.getAttachment()).isRobotSend())) {
        ...
        private void onVoiceToText(IMMessage item) {
            if (voiceTrans == null) voiceTrans = new VoiceTrans(container.activity);
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    public void sendReceipt() {
        ...
        //处理本地已读消息
        if (message != null) {
            updateUnreadMessage(message.getRemoteExtension().get("orderNo").toString());
        } else {
            updateUnreadMessage(P2PMessageActivity.orderNo);
        }
    ...
    private void updateUnreadMessage(String orderNo) {
        if (TextUtils.isEmpty(orderNo)) {
            return;
        }
        removeMessages = new ArrayList<>();
        for (IMMessage imMessage : IMMessageEntity.getMessages()) {
            if (imMessage.getRemoteExtension().get("orderNo").toString().
                    equals(orderNo)) {
                removeMessages.add(imMessage);
            }
        }
        IMMessageEntity.getMessages().removeAll(removeMessages);
    ...
    private boolean sendReceiptCheck(final IMMessage msg) {
        return msg != null && msg.getDirect() == MsgDirectionEnum.In && msg.getMsgType() != MsgTypeEnum.tip && msg.getMsgType() != MsgTypeEnum.notification;
    }
